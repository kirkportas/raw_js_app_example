var auth = {
    checkToken: function () {
        api.getToken()
            .done(function () {
                if (GFT_APP.DEBUG) { console.info('Token is valid, we are not going to refresh it'); }
    ...
    handleLogin: function () {
        if (GFT_APP.DEBUG) { console.info('Trying to sign in'); }
        ...
        if (username.length === 0 || password.length === 0) {
            if (GFT_APP.DEBUG) { console.error('Did not provide both username and password'); }
            PhoneGapProxy.navigator.notification.alert('Please enter both a username and password');
            $loginButton.removeAttr('disabled');
            return;
        }

        if (GFT_APP.DEBUG) { console.log('Sending ' + username + ' (username) and ' + password + ' (password)'); }

        api.authorize({
            username: username,
            password: password
        }).done(function () {
                if (GFT_APP.DEBUG) { console.info('Signed in!'); }
                $('#loginPage').hide();
                app.showInventoryListView();
            }).fail(function () {
                // TODO error handling for offline / wrong credentials should happen here not in api.js
                //if (GFT_APP.DEBUG) { console.error('Login failed'); }
                //PhoneGapProxy.navigator.notification.vibrate();
